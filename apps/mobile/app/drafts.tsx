...
export default function DraftsScreen() {
  const returnToAccount = useCallback(() => router.replace("/account"), []);
  ...
  return <Screen><Pressable accessibilityRole="link" accessibilityLabel="Back to account" onPress={returnToAccount} style={({ pressed }) => [styles.row, { alignSelf: "flex-start", paddingVertical: 4, opacity: pressed ? 0.72 : 1 }]}><Ionicons name="chevron-back" size={19} color={colors.brand} /><Text style={{ color: colors.brand, fontSize: 15, fontWeight: "800" }}>Back to account</Text></Pressable><View style={{ gap: 6, paddingTop: 4 }}><Eyebrow>Your writing desk</Eyebrow><Title>Drafts</Title><Subtitle>Work privately until a note is ready to publish.</Subtitle></View><Action title="New draft" onPress={() => router.push("/draft/new")} icon={<Ionicons name="create-outline" size={19} color="#fff" />} />
