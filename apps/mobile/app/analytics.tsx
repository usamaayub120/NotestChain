...
export default function AnalyticsScreen() {
  const query = useQuery({ queryKey: ["analytics"], queryFn: async () => { try { const result = await apiPage<Analytics>("/publications/mine/analytics?page=1&pageSize=25"); cacheWrite("analytics", result); return result.data; } catch { const cached = cacheRead<Page<Analytics> | Analytics[]>("analytics"); return Array.isArray(cached) ? cached : cached?.data ?? []; } } });
