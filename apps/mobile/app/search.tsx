...
export default function SearchScreen() {
  ...
  const search = async () => { if (!term.trim()) return; setLoading(true); setError(undefined); try { setResult(await apiPage<Publication>(`/search?q=${encodeURIComponent(term.trim())}&page=1&pageSize=30&sort=relevance`)); } catch (e) { setError(e instanceof Error ? e.message : "Search failed."); } finally { setLoading(false); } };
